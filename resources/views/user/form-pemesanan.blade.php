...
<x-app-layout>
                    ...
                    <form action="{{ route('pemesanan.store', $mobil->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <p class="mt-2 text-sm text-gray-500">
                                <span class="inline-block w-3 h-3 bg-red-300 rounded-full mr-1 align-middle"></span> 
                                Tanggal tidak tersedia (sudah dipesan)
                            </p>
                        </div>
                        ...
                        <!-- Kolom Alamat Lengkap yang ditambahkan -->
                       <div class="mt-4" id="alamat_pengantaran" style="display: none;">
    <label for="alamat_pengantaran" class="block text-sm font-medium text-gray-700">Alamat Pengantaran</label>
    <input type="text" name="alamat_pengantaran" id="alamat_pengantaran"
        ...
        value="{{ old('alamat_pengantaran') }}">
                        ...
                        <!-- Penjelasan Ambil di Tempat -->
                        <div id="infoAmbil" class="mt-4 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 hidden">
                            <h4 class="font-semibold mb-2">Informasi Pengambilan di Tempat:</h4>
                            <ul class="list-disc ml-5 text-sm">
                                <li>Harap datang tepat waktu sesuai tanggal dan jam yang telah disepakati.</li>
                                <li>Tunjukkan bukti pemesanan serta KTP dan SIM asli saat pengambilan.</li>
                                <li>Pastikan kendaraan dicek sebelum meninggalkan lokasi.</li>
                            </ul>
                            <p class="mt-2 text-sm font-medium">Alamat Pengambilan:</p>
                            <p class="text-sm">Jl. Jenderal Sudirman No 385, Kelurahan Kayu Embun, Kecamatan Padang Sidempuan Utara, Kota Padang Sidempuan</p>
                        </div>
                ...
                <script>
                    ...
                    btnBatal.addEventListener('click', () => modalKonfirmasi.classList.remove('hidden'));
                    cancelBatal.addEventListener('click', () => modalKonfirmasi.classList.add('hidden'));
                    ...
                    document.addEventListener('DOMContentLoaded', () => {
                        ...
                        const infoAmbil = document.getElementById('infoAmbil');

                        function toggleInfoAmbil() {
                            if (pengambilanSelect.value === 'ambil') {
                                infoAmbil.classList.remove('hidden');
                            } else {
                                infoAmbil.classList.add('hidden');
                            }
                        }

                        pengambilanSelect.addEventListener('change', toggleInfoAmbil);
                        toggleInfoAmbil(); // Set saat halaman pertama kali dimuat
                    });
                </script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const pengambilanSelect = document.getElementById('pengambilan');
        const infoAmbil = document.getElementById('infoAmbil');
        const alamatField = document.getElementById('alamat_pengantaran');

        function togglePengambilanFields() {
            const value = pengambilanSelect.value;
            if (value === 'ambil') {
                infoAmbil.classList.remove('hidden');
        ...
        pengambilanSelect.addEventListener('change', togglePengambilanFields);

        // Inisialisasi kondisi awal saat halaman dimuat
        togglePengambilanFields();
    });
</script>
